...
func printTable(wfList []wfv1.Workflow, out io.Writer, opts PrintOpts) {
	...
	if !opts.NoHeaders {
		...
		_, _ = fmt.Fprint(w, "NAME\tSTATUS\tAGE\tDURATION\tPRIORITY\tCOST")
		if opts.Output == "wide" {
	...
	for _, wf := range wfList {
		...
		_, _ = fmt.Fprintf(w, "%s\t%s\t%s\t%s\t%d$%f", wf.ObjectMeta.Name, WorkflowStatus(&wf), ageStr, durationStr, priority, workflowCost.TotalCost)
		if opts.Output == "wide" {
